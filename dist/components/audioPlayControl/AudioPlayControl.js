...
export class AudioPlayControl extends PureComponentSkipFunction {
    ...
    _renderDefault() {
        return (<Row style={this.props.style} dial={4}>
                {this._renderPlayBtn()}
    ...
    _renderPlayBtn(style) {
        return (<BtnPausePlay ref={(ref) => { this.btnPlay = ref; }} style={style} onPress={this.btnPlayPauseClick.bind(this)}/>);
    }
    _renderSeekbarAudioPlayer(style) {
        return (<SeekBarAudioPlayer ref={(ref) => { this.seekBarPlayer = ref; }} style={style} onValueChange={this._seekValueChange.bind(this)}/>);
    }
    //region Loop ======
    btnLoopClick() {
    ...
    _renderLoopBtn() {
        if (this.props.showLoopBtn) {
            return <Touchable style={{ justifyContent: "center", alignItems: "center", paddingLeft: 10, paddingRight: 10 }} onPress={this.btnLoopClick.bind(this)}>
    ...
    _renderSpeedBtn() {
        if (!this.props.showSpeedBtn || !NativeModules.RNSound.IsEnableSpeed)
            return undefined;
        return <ComboBox listData={["1.3x", "1x", "0.8x", "0.7x", "0.6x", "0.5x"]} style={{ width: 50, borderWidth: 0, height: 30 }} indexSelected={1} selectedChange={this._selectSpeedChange.bind(this)}/>;
    }
}
AudioPlayControl.defaultProps = { showLoopBtn: true };
